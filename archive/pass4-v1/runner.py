...
async def run_one(entry: dict, resume: bool = True, archive: bool = True) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="pass4-v1 single-paper runner")
    ...
    for k in args.keys:
        try:
            r = asyncio.run(run_one(by_key[k], resume=not args.no_resume, archive=not args.no_archive))
            results.append(r)
        except Exception as exc:
            import traceback
            traceback.print_exc()
